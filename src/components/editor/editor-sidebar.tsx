...
import { Plus, GripVertical, User, FileText, Briefcase, GraduationCap, Wrench, FolderKanban, Award, Languages, LayoutList, Pencil, GitBranch, QrCode } from 'lucide-react';
...
const sectionIcons: Record<string, React.ElementType> = {
  ...
  internship_experience: Briefcase,
  education: GraduationCap,
  skills: Wrench,
  projects: FolderKanban,
  certifications: Award,
  self_evaluation: FileText,
  languages: Languages,
  github: GitBranch,
  qr_codes: QrCode,
  custom: LayoutList,
  ...
  return (
    ...
    >
      ...
      {isRenamable && !isRenaming && (
        <button
          type="button"
          aria-label={`重命名 ${section.title}`}
          className="shrink-0 cursor-pointer rounded p-0.5 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200"
  ...
  const handleAddSection = (type: SectionType) => {
    const newSection: ResumeSection = {
      ...
      content: type === 'personal_info'
        ...
        : type === 'summary' || type === 'self_evaluation'
          ? { text: '' }
          : type === 'skills'
            ? { categories: [] }
            : { items: [] },
