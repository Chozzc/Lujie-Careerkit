...
export function PagedResumePreview({ resume, zoom, smartOnePage }: PagedResumePreviewProps) {
  ...
  useLayoutEffect(() => {
    const measure = () => {
      ...
      const nextCandidates = collectResumePageBreakCandidates(node);
      setBreakCandidates((current) =>
        current.length === nextCandidates.length && current.every((value, index) => value === nextCandidates[index])
          ? current
          : nextCandidates,
      );
    };
    ...
    <div className="relative flex min-h-full justify-center p-3 md:p-5">
      ...
      <div className="flex flex-col items-center gap-7">
        ...
        {pagination.pageSlices.map((pageSlice, pageIndex) => (
          <div key={`${pageSlice.sourceStart}-${pageSlice.sourceEnd}`} className="flex flex-col items-center gap-2">
            <div
