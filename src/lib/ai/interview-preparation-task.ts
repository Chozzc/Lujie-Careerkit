...
export function buildInterviewPreparationPrompt(input: InterviewPreparationInput) {
  ...
  return [
    ...
    "",
    "工作流程：",
    "1. 先按“岗位职能 × 经验级别 × 行业领域”识别岗位，不要因为公司行业而误判岗位职能。",
    "2. 拆解核心职责、硬性要求、加分项、交付结果、协作方式和领域知识。",
    "3. 提炼 5-7 个简短、互不重复的能力维度，形成 capabilityProfile。requirementLevel 只表示 JD 优先级，evidenceLevel 只表示简历证据强弱，不等于候选人的真实能力。",
    "4. 对重要要求逐条寻找简历证据，状态只能是 direct、transferable、not-shown、gap、confirm。",
    "5. 生成 3-8 个真正与 JD 相关的核心知识点，给出简明解释、掌握目标和自测问题。",
    "6. 从简历中最多选择 2-4 段最可能被追问的真实经历，准备个人贡献、决策、方法、证据、结果和复盘方向；没有可用经历时返回空数组，不要凑数。",
    "7. 给出 6-10 道针对性问题及准备方向，但不要冒充公司真实题库。",
    "8. 把行动分为 mustPrepare、shouldPrepare、optional，保证具体、可执行。",
    "",
    "文档结构：",
    "- meta：岗位与资料概览。",
