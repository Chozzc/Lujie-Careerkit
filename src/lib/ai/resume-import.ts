      ...
      messages: [
        {
          role: "system",
          content:
            "你是简历信息抽取助手，不是简历优化助手。只做结构识别和字段归类，保留原文语义、原句和顺序，不要润色、压缩、改写、重排或补写内容；但不要输出项目符号、列表编号、色块、装饰线、页码等版式噪声。只输出一个合法 JSON 对象，不要 Markdown，不要解释。",
        },
        {
          role: "system",
...
function buildResumeImportPrompt(fileName: string) {
  return [
    ...
    "重要：这是“导入并还原简历”，不是“优化简历”。请保留用户原句、原有小标题和原顺序，只删除明显的项目符号、列表编号、色块、装饰线、页眉页脚、页码和多余空白。",
    "",
    "字段要求：",
    ...
    "- projects：项目经历。name/role/highlights 必须存在。highlights 必须保留原简历 bullet 后面的正文和原顺序，不要保留 bullet 符号、编号、色块，不要合并、删减、润色或重新排序。",
    ...
    "- 不要输出“说明”、红色方块、黑色圆点、序号 1/2/3、9/10/11、页眉页脚、页码、装饰线等 OCR 或版式噪声；如果它们只是列表样式，删除符号并保留后面的正文。",
