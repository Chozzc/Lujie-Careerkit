...
export function calculateResumePagination(
  ...
  const subpixelOverflowAdjusted = Boolean(
    !smartOnePageApplied && measuredHeight > A4_HEIGHT_PX && measuredHeight <= A4_HEIGHT_PX + SUBPIXEL_OVERFLOW_PX,
  );
  const fitScale = (smartOnePageApplied || subpixelOverflowAdjusted) && measuredHeight > A4_HEIGHT_PX
    ? Math.min(1, neededScale)
    : 1;
  const pageSlices = smartOnePageApplied || subpixelOverflowAdjusted
...
function calculatePageSlices(contentHeight: number, breakCandidates: number[]): ResumePageSlice[] {
  ...
  while (sourceStart < contentHeight) {
...
export function collectResumePageBreakCandidates(root: HTMLElement): number[] {
  ...
  const rootHeight = Math.max(root.scrollHeight, rootRect.height, 1);
  const candidates: number[] = [];
  const textIntervals: Array<{ top: number; bottom: number }> = [];

  const addTextRect = (rect: DOMRect) => {
  ...
  if (nodeFilter) {
    ...
    while (node) {
      if (node.textContent?.trim()) {
        ...
        Array.from(range.getClientRects()).forEach(addTextRect);
      }
  ...
  for (let index = 0; index < mergedTextIntervals.length - 1; index += 1) {
    ...
    if (next.top > current.bottom) candidates.push((current.bottom + next.top) / 2);
  }

  return [...new Set(candidates.map((value) => Math.round(value * 2) / 2))].sort((a, b) => a - b);
}
